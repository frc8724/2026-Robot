...
public class RobotContainer {
        ...
        ratatouille.setDefaultCommand(
                ...
                ratatouille.applyRequest(
                        () -> {
                            ...
                            // Drive forward with negative Y (forward)
                            return drive.withVelocityX(-driverStick.getRawAxis(Axis.Y) * MaxSpeed * multiplier)
                                    // Drive left with negative X (left)
                                    .withVelocityY(-driverStick.getRawAxis(Axis.X)
                                            * MaxSpeed * multiplier)
    ...
    private void configureDebugBindings() {
        ...
        debugStick.Button(7).onFalse(new ParallelCommandGroup(hopper.setSpeedCommand(.0), loader.setSpeedCommand(.0)));
