...
public class RobotContainer {
    private double MaxSpeed = 0.3 * TunerConstants.kSpeedAt12Volts.in(MetersPerSecond); // kSpeedAt12Volts desired top
                                                                                        // speed
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                ...
                drivetrain.applyRequest(
                        ...
                        drive.withVelocityX(-driverStick.getRawAxis(Axis.Y) * MaxSpeed)
                                ...
                                .withVelocityY(-driverStick.getRawAxis(Axis.X) * MaxSpeed)
                                // Drive counter-clockwise with negative X (left)
                                .withRotationalRate(-driverStick.getRawAxis(Axis.Z) * MaxAngularRate)));
        ...
        // intake on and off
        operatorPad.Button(5).onTrue(new ParallelCommandGroup(intakeRollers.intakeCommand(), hopper.turnOnCommand()));
        operatorPad.Button(5)
                .onFalse(new ParallelCommandGroup(intakeRollers.turnOffCommand(), hopper.turnOffCommand()));

        operatorPad.Button(7).onTrue(new ParallelCommandGroup(intakeRollers.outtakeCommand(), hopper.reverseCommand()));
        ...
        climberElevatorPivot.setDefaultCommand(
                climberElevatorPivot
                        .controlWithAxis(operatorPad.Axis(frc.robot.controls.MayhemOperatorPad.Axis.RightX)));
        climberElevator.setDefaultCommand(
                climberElevator.controlWithAxis(operatorPad.Axis(frc.robot.controls.MayhemOperatorPad.Axis.LeftY)));
